...
//using PropertyDictionaryItemLocalizedValue = VirtoCommerce.CatalogPublishingModule.Test.Model.PropertyDictionaryItemLocalizedValue;
...
//            Id = "Valid",
//            CatalogId = "Valid",
//            Properties = new List<Property>().Cast<CatalogModule.Core.Model.Property>().ToList(),
//            PropertyValues = new List<PropertyValue>().Cast<CatalogModule.Core.Model.PropertyValue>().ToList(),
//            Reviews = new List<EditorialReview>().Cast<CatalogModule.Core.Model.EditorialReview>().ToList(),
...
//            // catalog
//            await Assert.ThrowsAsync<ArgumentNullException>(() => evaluator.EvaluateCompletenessAsync(null, new[] { new CatalogProduct() }));
//            // products
//            await Assert.ThrowsAsync<ArgumentException>(() => evaluator.EvaluateCompletenessAsync(new CompletenessChannel(), null));
//            await Assert.ThrowsAsync<ArgumentException>(() => evaluator.EvaluateCompletenessAsync(new CompletenessChannel(), new CatalogProduct[0]));
//        }

//        [Fact]
//        public async Task PartiallyLoadedProducts()
//        {
//            var evaluator = GetCompletenessEvaluator();
//            await evaluator.EvaluateCompletenessAsync(_channel, new[] { new CatalogProduct { Id = "Valid" } });
//        }
...
//                        Dictionary = true,
//                        DictionaryValues = new[] { new PropertyDictionaryItemLocalizedValue { Value = dictionaryPropertyValues[PropertyValueType.ShortText][0] } }
//                    },
...
//                        Multilanguage = true,
//                        Dictionary = true,
//                        DictionaryValues = new[] { new PropertyDictionaryItemLocalizedValue { LanguageCode = "Valid", Value = dictionaryPropertyValues[PropertyValueType.ShortText][0] } }
//                    },
...
//                foreach (var dictionaryValue in new[] { null, new PropertyDictionaryItemLocalizedValue[0] })
//                {
//                    var currentProperty = property;
...
//                            .Select((x, i) => new PropertyDictionaryItemLocalizedValue { Value = dictionaryPropertyValues[PropertyValueType.ShortText][i] })
//                            .ToArray()
//                })
//                    .ToList();
...
//                            .Select((x, i) => new PropertyDictionaryItemLocalizedValue { LanguageCode = l, Value = dictionaryPropertyValues[PropertyValueType.ShortText][i] }))
//                            .ToArray()
//                })
//                    .ToList();
...
//            _product.Properties = properties.Cast<CatalogModule.Core.Model.Property>().ToList();
//            _product.PropertyValues = values.Cast<CatalogModule.Core.Model.PropertyValue>().ToList();
...
//            service.Setup(x => x.SearchPricesAsync(It.IsAny<PricesSearchCriteria>()))
//                .ReturnsAsync(c => new PricingSearchResult<PricingModule.Core.Model.Price> { Results = _prices?.Where(p => p.PricelistId == c.PriceListId).ToArray() });
//            return service.Object;
//        }
...
//            service.Setup(x => x.GetObjectSettingAsync(It.Is<string>(n => n == editorialReviewTypesSettingName), It.IsAny<string>(), It.IsAny<string>()))
//                .ReturnsAsync<string>(x => new ObjectSettingEntry { AllowedValues = _editorialReviewTypes });
//            return service.Object;
//        }
//    }
//}
